...
async function api<T>(url: string, init?: RequestInit): Promise<T> {
  const response = await fetch(sitePath(url), { ...init, headers: { "Content-Type": "application/json", ...init?.headers } });
...
export default function Home() {
  ...
  async function submitAuth() {
    try {
      const result = await api<{ user: Account }>("/api/auth/login", { method: "POST", body: JSON.stringify({ password: authValue }) });
  ...
  async function importReadingMedia(files: File[], message: string, origin: "knowledge" | "canvas" = "knowledge") {
    ...
    for (const file of files) {
      if (!file.type.startsWith("image/")) {
        ...
        const response = await fetch(sitePath("/api/reading/import-document"), { method: "POST", body: form });
      ...
      for (let attempt = 0; attempt < 2; attempt += 1) {
        ...
        try {
          const response = await fetch(sitePath("/api/reading/import-media"), { method: "POST", body: form, signal: controller.signal });
  ...
  async function listReadingCanvases() {
    const result = await api<{ canvases: { name: string }[] }>("/api/reading/canvas");
    return result.canvases;
  }

  async function createReadingCanvas(name: string) {
  ...
  if (!account) return <main className="auth-page"><section className="auth-card"><span className="brand-mark">S</span><small>PRIVATE · SINGLE ACCOUNT</small><h1>Slowday</h1><p>这是你的私人日历、复盘与品牌资料库。</p><label>网站密码<input autoFocus type="password" autoComplete="current-password" value={authValue} onChange={e => setAuthValue(e.target.value)} onKeyDown={e => e.key === "Enter" && void submitAuth()} placeholder="输入网站密码" /></label><button className="primary-btn wide" onClick={() => void submitAuth()}>进入 Slowday</button><small className="auth-note">此网站只开放一个账户，登录后数据会自动同步到云端。</small></section>{error && <button className="error-toast" onClick={() => setError("")}><b>没有完成</b><span>{error}</span><i>×</i></button>}</main>;
            ...
            <div className="sidebar-todos">
              {selectedTasks.length ? selectedTasks.map(task => (
                <div key={task.id} className={`side-todo ${task.done ? "done" : ""}`}>
                  {editingTaskId === task.id ? <div className="side-todo-editor inline-task-editor" onBlur={event => handleTaskEditorBlur(event, task)}><input className="inline-task-title" autoFocus value={editingTaskTitle} onChange={event => setEditingTaskTitle(event.target.value)} onKeyDown={event => { if (event.key === "Enter") event.currentTarget.blur(); if (event.key === "Escape") setEditingTaskId(null); }} aria-label="修改待办内容" /><label className="inline-task-date" title="修改日期"><CalendarIcon /><input className="inline-task-date-input" type="date" value={editingTaskDate} onChange={event => setEditingTaskDate(event.target.value)} aria-label="修改待办日期" /></label></div> : <><button className="todo-check-only" aria-label={task.done ? "标记为未完成" : "标记为已完成"} onClick={event => { event.stopPropagation(); void toggleTask(task.id); }}><i>{task.done ? "✓" : ""}</i></button><button className="todo-title-edit" onClick={event => { event.stopPropagation(); beginTaskEdit(task); }} aria-label={`编辑待办：${task.title}`}>{task.title}</button></>}
                  <span className="todo-tools"><button aria-label="删除待办" onClick={event => { event.stopPropagation(); void deleteTask(task); }}>×</button></span>
                </div>
          ...
          <section className="review-panel">
            ...
            <div className="review-layout">
              ...
              <aside className="review-side">
                ...
                <div className="task-summary"><small>今日待办</small><h3>{selectedTasks.filter(t => t.done).length} / {selectedTasks.length} 已完成</h3>{selectedTasks.length ? selectedTasks.map(task => <div key={task.id} className={`task-summary-row ${task.done ? "done" : ""}`}><button className="task-summary-check" aria-label={task.done ? "标记为未完成" : "标记为已完成"} onClick={() => void toggleTask(task.id)}><i>{task.done ? "✓" : ""}</i></button>{editingTaskId === task.id ? <div className="task-summary-editor inline-task-editor" onBlur={event => handleTaskEditorBlur(event, task)}><input className="inline-task-title" autoFocus value={editingTaskTitle} onChange={event => setEditingTaskTitle(event.target.value)} onKeyDown={event => { if (event.key === "Enter") event.currentTarget.blur(); if (event.key === "Escape") setEditingTaskId(null); }} aria-label="修改待办内容" /><label className="inline-task-date" title="修改日期"><CalendarIcon /><input className="inline-task-date-input" type="date" value={editingTaskDate} onChange={event => setEditingTaskDate(event.target.value)} aria-label="修改待办日期" /></label></div> : <button className="task-summary-title" onClick={() => beginTaskEdit(task)}>{task.title}</button>}</div>) : <p>今天还没有添加待办。</p>}</div>
